...
arima.aic.bic <- function(model.list, seasonal = TRUE) {
  ...
  for (name in names(model.list)) { # 遍历列表的名称
    ...
    # if (seasonal) {
    #   if (model$call$seasonal[[1]] != "list") {
    #     temp$P <- model$call$seasonal[[2]]
    #     temp$D <- model$call$seasonal[[3]]
    #     temp$Q <- model$call$seasonal[[4]]
    #   } else {
    #     temp$P <- model$call$seasonal$order[[2]]
    #     temp$D <- model$call$seasonal$order[[3]]
    #     temp$Q <- model$call$seasonal$order[[4]]
    #   }
    # }
...
arima.tendency.chart <- function(data, observe, title = "", breaks_by = "year") {
  ...
  p <- ggplot(data, aes(x = date, y = .data[[observe]])) +
    ...
    geom_ribbon(aes(ymin = lower_bound, ymax = upper_bound, fill = "Confidence Interval"), alpha = 0.3) + # 添加置信区间),
    # 添加置信区间
    scale_x_date(
...
arima.stationary.test <- function(observe, d = 0, D = 0, seasonal.unit = c("Month", "Season", "Year")) {
  diff_data <- observe
  # 添加映射关系
  circle <- 0
  if (seasonal.unit == "Month") {
    circle <- 1
  } else if (seasonal.unit == "Season") {
    circle <- 4
  } else if (seasonal.unit == "Year") {
    circle <- 12
  }

  # 对观察值进行差分
  if (d != 0) {
    diff_data <- diff(diff_data, lag = d * circle)
  }
  if (D != 0) {
    diff_data <- diff(diff_data, differences = D)
  }
...
#' TODO: 执行 ARIMA 模型的白噪声检验
#' @description
#' 对给定的残差序列进行白噪声检验，检查残差是否为白噪声。
#' @param observed T: numeric 向量，观察值序列
#' @returns T: list 返回白噪声检验的结果，包括统计量和p值
#' @export
#'
#' @examples
#' noise_test <- arima.noise.test(model.list$fit.2$residuals)
arima.noise.test <- function(observed) {
  # 检查残差是否为白噪声
  noise_test <- Box.test(observed, lag = 20, type = "Ljung-Box")
  return(noise_test)
}
...
#' 生成 ARIMA 模型的参数列表
#' @description
#' 该函数用于生成 ARIMA 模型的参数列表，包括 `order` 和 `seasonal` 两个部分。
#' `order` 部分包含 ARIMA 模型的阶数参数，`seasonal` 部分包含季节性参数。
#' @param order T: vector<int> ARIMA 模型的阶数参数，包括 p, d, q 三个参数。
#' @param seasonal T: vector<int> 季节性参数，包括 P, D, Q 三个参数。
#' @param period T: int 季节性周期，即数据的周期性间隔，默认为 12（即每年 12 个观测值）。
#' @returns T: list 返回 ARIMA 模型的参数列表，包括 `order` 和 `seasonal` 两个部分。
#' @export
#' @examples
#' # 生成 ARIMA 模型的参数列表
#' arima.arg(c(1, 1, 1), c(1, 1, 1), period = 12)
arima.arg <- function(order, seasonal = NULL, period = 12) {
  # 处理seasonal为NULL的情况
  if (is.null(seasonal)) {
    return(list(
      order = order,
      seasonal = NULL,
      description = sprintf("(%s)", paste(order, collapse = ", "))
    ))
  } else {
    return(list(
      order = order,
...
#' 将 ARIMA 模型参数列表转换为模型对象列表
#' @description
#' 该函数用于将 ARIMA 模型的参数列表转换为 ARIMA 模型对象列表。适用于季节性生成
#' @param arg.list T: list 包含 ARIMA 模型参数的列表。
#' @param data T: ts 时间序列数据。
#' @returns T: list 返回 ARIMA 模型对象列表。
#' @export
#' @examples
#' # 生成 ARIMA 模型参数列表
#' arg.list <- list(
#'   arima.arg(c(1, 1, 1), c(1, 1, 1), period = 12),
#'   arima.arg(c(1, 1, 1), c(1, 1, 2), period = 12)
#' )
arima.convert.to.model.list <- function(arg.list, data) {
  model.list <- list()
  for (i in seq_along(arg.list)) {
    arg <- arg.list[[i]]
    if (is.null(arg$seasonal$order)) {
      model.list[[paste0("fit.", i, arg$description)]] <- Arima(
        data,
        order = arg$order
      )
      next
...
#' 通过取得的arima.aic.bic表的结果，生成表中提到的全部模型的系数查看列表
#' @description
#' 该函数用于通过取得的arima.aic.bic表的结果，生成表中提到的全部模型的系数查看列表
#' @param model.name.vector T: vector<char> 备选模型的名称列表
#' @returns T: list 返回包含所有模型系数查看列表的列表
#' @export
#' @examples
#' # 生成模型系数查看列表
#' coef.se.list <- arima.coef.se.list.builder(model.name.vector)
arima.coef.se.list.builder <- function(model.name.vector) {
  coef.se.list <- list()
  for (model.name in model.name.vector) {
    coef.se.list[[model.name]] <- arima.coef.se(model.list[[model.name]])
  }
  return(coef.se.list)
}
